        ...
        try:
            ...
            for attempt in range(retry_attempts + 1):
                try:
                    ...
                    print(f"[Chatflow处理] 开始提取chatflow_answer")
                    chatflow_answer = sse_client.extract_chatflow_answer(all_events)
                    print(f"[Chatflow处理] 提取到的chatflow_answer: {chatflow_answer}")
                    
                    # Chatflow专用处理：过滤关键事件
                    print(f"[Chatflow处理] 开始过滤关键事件")
                    key_events = sse_client.filter_key_events(all_events)
                    print(f"[Chatflow处理] 过滤出{len(key_events)}个关键事件")
                    ...
                    print(f"[最终结果] 构建完成，包含{len(all_events)}个事件，{len(key_events)}个关键事件")
